...
def _sanitize_windows_filename(name: str, max_len: int = 80) -> str:
    """
    Make a string safe to use as a Windows file-name component.
...
class Hy3DExportMesh:
    ...
    def process(self, trimesh, filename_prefix, file_format, save_file=True):
        # Let Comfy decide folder + filename based on prefix (like original code)
        ...
        safe_filename = _sanitize_windows_filename(filename, max_len=60)

        # Main target path
        output_glb_path = Path(
            full_output_folder,
            f"{safe_filename}_{counter:05}_.{file_format}"
        )
        output_glb_path.parent.mkdir(parents=True, exist_ok=True)

        # Fallback if Windows rejects the path
        fallback_name = _sanitize_windows_filename("hy3d_export", max_len=32)
        fallback_path = Path(
            full_output_folder,
            f"{fallback_name}_{counter:05}_.{file_format}"
        )

        def _export_to_path(mesh_obj, target: Path):
            """
            Export via trimesh to data, then write ourselves.
            Avoids using path directly in trimesh.export(path, ...).
            """
            data = mesh_obj.export(file_type=file_format)

            # GLB/OBJ may return bytes or str
        ...
        # For debugging: you can uncomment this to see where it's trying to write
        print("[Hy3DExportMesh] Exporting to:", str(output_glb_path))

        if save_file:
            try:
                _export_to_path(trimesh, output_glb_path)
                relative_path = Path(subfolder) / output_glb_path.name
            except OSError as e:
                if e.errno == 22:
                    _export_to_path(trimesh, fallback_path)
                    relative_path = Path(subfolder) / fallback_path.name
                else:
                    raise
        else:
            temp_file = Path(full_output_folder, f"hy3dtemp_.{file_format}")
            try:
                _export_to_path(trimesh, temp_file)
                relative_path = Path(subfolder) / temp_file.name
            except OSError as e:
                if e.errno == 22:
                    # last-resort simple file at output root
                    root_temp = Path(
                        folder_paths.get_output_directory(),
                        f"hy3dtemp_{counter:05}_.{file_format}"
                    )
                    _export_to_path(trimesh, root_temp)
                    relative_path = root_temp.relative_to(
                        folder_paths.get_output_directory()
                    )
                else:
                    raise
